...
mydb = mysql.connector.connect(host="localhost", user="root", passwd="",port=3306, database="e-agri")
cursor = mydb.cursor()
...
def cropback():
    if request.method=='POST':
        ...
        msg = rfp[0]
        print(msg)
        flash("Future estimation crop yeild is ", "info")
...
def investback():
    if request.method=='POST':
        ...
        if m < invest:
            ...
            # sender_address1 = 'cse.Takeoff@gmail.com'
            # sender_pass1 = 'Takeoff@123'
...
def sellback():
    ...
    if request.method=='POST':
        ...
        sql = "INSERT INTO crop_selling (fname,email,pno,addr,cname,q) VALUES (%s,%s,%s,%s,%s,%s)"
...
def viewcrop():
    ...
    x = pd.read_sql_query(sql, mydb)
    x = x.drop(['fname', 'email','pno','addr','cname','quantity','status'], axis=1)
...
def buyback():
    if request.method=="POST":
        ...
        pno=session.get('pno')
        print(name)
        sql = "INSERT INTO selling_buying (id,fname,femail,bname,bemail,pno,addr,cname,quantity,buyer_need) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
